package crystal.somewhere.utils;
...
/**
 * Created by zhoulin on 2018/1/6.
 */
...
public class StringUtils {
    ...
    /**
     * @param type tags集合
     * @description 切割字符串，string 类型的type变成list<String>
     */
    public static List<String> splitTypeToTags(String type) {
        List<String> splitedTags = new ArrayList<String>();
        String[] temp = type.split("\\|");
        for(int i = 0; i < temp.length; ++i) {
            System.out.println("temp"+i+": "+ temp[i]);
            splitedTags.add(temp[i]);
        }
        return splitedTags;
    }

    //list<string>转长string
    public static String listToString(List<String> tl) {
        StringBuffer longString = new StringBuffer();
        for (int i = 0; i < tl.size(); ++i) {
            longString.append(tl.get(i));
            longString.append("|");
        }
